...
=====================
Upload reverse_dcf.xlsx → interactive analysis with adjustable parameters.
...
if model is None:
    ...
    st.info("Upload your reverse_dcf.xlsx file (with hard-copy data filled in) to start.")
    st.markdown("""
    **Workflow:**
    1. Open `reverse_dcf.xlsx` on Bloomberg Terminal
    2. Change ticker in `Fundamentals!B1`
    3. Copy BBG values (green cells) into Hard Copy cells (yellow cells)
    4. Fill Current sheet the same way
    5. Update Macro & WACC tabs if needed
    6. Save and upload here
    """)
    st.stop()

# ── WACC Parameters (override from Excel defaults) ───────────────────────────
st.sidebar.markdown("---")
...
tm = st.sidebar.slider("Terminal EBIT Margin (%)", 5.0, 35.0,
                       round((p.terminal_ebit_margin or 0.15)*100, 1), 0.5) / 100
...
# Apply overrides
params = DCFParams(
    risk_free=rf, erp=erp, beta=beta, cost_of_debt_pretax=cod, tax_rate=tax,
    equity_weight=we, debt_weight=1-we, terminal_growth=tg,
    terminal_ebit_margin=tm, projection_years=proj,
    bull_growth_add=bull, bear_growth_add=bear,
)
model.params = params
model._prepare_data()
r = model.run()
...
k2.metric("Implied Growth", f"{r['implied_growth']:.1%}",
          delta=f"vs {r['historical_profile'].revenue_cagr_5y:.1%} 5Y" if r['historical_profile'].revenue_cagr_5y else None)
k3.metric("WACC", f"{r['wacc']:.2%}")
k4.metric("TV % of EV", f"{r['tv_decomposition']['tv_pct']:.0%}")
spread = r['roic_gate']['spread']
k5.metric("ROIC Spread", f"{spread:+.1%}",
...
with cr:
    ...
    rg = r["roic_gate"]
    st.write(rg["verdict"])
    if not np.isnan(rg.get("implied_reinvestment_rate", np.nan)):
        st.write(f"Implied reinvestment rate: **{rg['implied_reinvestment_rate']:.0%}**")
...
# ── Sensitivity ───────────────────────────────────────────────────────────────
st.markdown("---")
st.subheader("Sensitivity: Implied Growth (WACC x Terminal Growth)")
...
st.dataframe(sdf.style.format("{:.1%}").background_gradient(cmap="RdYlGn", axis=None),
...
if warnings:
    st.markdown("---")
    with st.expander(f"Data Validation ({len(warnings)} warnings)", expanded=True):
        for w in warnings:
...
with st.expander("Historical Data"):
    st.dataframe(model.hist, use_container_width=True)
with st.expander("Current Snapshot"):
    st.json({k: str(v) for k, v in model.current.items()})
